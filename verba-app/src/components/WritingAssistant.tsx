...
export function WritingAssistant({ 
  ...
  const handleGenerateContextual = async (customInstruction?: string) => {
    const finalInstruction = customInstruction || instruction;
    if (!contextualSelection || !finalInstruction.trim()) return;
  ...
  if (contextualSelection) {
    return (
      <div className="flex flex-col h-full bg-white relative font-sans">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border-light bg-white sticky top-0 z-10 shrink-0">
          <h3 className="text-[13px] font-semibold text-[#0B1628] uppercase tracking-wider">
            Contextual Actions
          </h3>
          <button onClick={onClearContextualSelection} className="text-foreground-muted hover:text-[#0B1628] transition-colors">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="p-5 flex-1 overflow-y-auto space-y-8">
          <section>
            <h4 className="text-[11px] font-semibold text-foreground-muted uppercase tracking-wider mb-3">Selected Text</h4>
            <div className="text-[13px] text-[#0B1628] pl-3 border-l-2 border-border-light italic">
              "{contextualSelection.originalText}"
            </div>
          </section>
          
          <hr className="border-border-light/50" />

          <section>
            <h4 className="text-[11px] font-semibold text-foreground-muted uppercase tracking-wider mb-3">Actions</h4>
            <div className="space-y-2">
              {showAskInput ? (
                <div className="bg-white border border-border-light rounded-md p-3">
                  <textarea
                    placeholder="Ask Verba to..."
                    value={instruction}
                    onChange={(e) => setInstruction(e.target.value)}
                    className="w-full min-h-[80px] text-[13px] focus:outline-none resize-none bg-transparent"
                    autoFocus
                  />
                  <div className="flex justify-end gap-2 mt-2">
                    <button onClick={() => setShowAskInput(false)} className="text-[12px] text-foreground-secondary hover:text-foreground">Cancel</button>
                    <button 
                      onClick={() => handleGenerateContextual()}
                      disabled={!instruction.trim() || isGeneratingContextual}
                      className="text-[12px] bg-accent text-white px-3 py-1.5 rounded flex items-center gap-1.5 disabled:opacity-50"
                    >
                      {isGeneratingContextual && <Loader2 size={12} className="animate-spin" />}
                      Generate
                    </button>
                  </div>
                </div>
                ...
                <>
                  <button 
                    ...
                    className="w-full flex items-center justify-between p-2.5 bg-white border border-border-light rounded-md hover:border-accent hover:text-accent transition-colors group"
                  >
                    <div className="flex items-center gap-2.5">
                      <Sparkles size={14} className="text-foreground-secondary group-hover:text-accent" />
                      <span className="text-[13px] font-medium text-[#0B1628] group-hover:text-accent">Ask Verba</span>
                    </div>
                    <ArrowRight size={14} className="opacity-0 group-hover:opacity-100 transition-opacity" />
                  </button>

                  <button 
                    onClick={() => handleGenerateContextual("Improve clarity and flow")}
                    className="w-full flex items-center justify-between p-2.5 bg-white border border-border-light rounded-md hover:border-[#9333EA] hover:text-[#9333EA] transition-colors group"
                  >
                    <div className="flex items-center gap-2.5">
                      <Edit2 size={14} className="text-foreground-secondary group-hover:text-[#9333EA]" />
                      <span className="text-[13px] font-medium text-[#0B1628] group-hover:text-[#9333EA]">Improve clarity</span>
                    </div>
                    <ArrowRight size={14} className="opacity-0 group-hover:opacity-100 transition-opacity" />
                  </button>

                  <button 
                    onClick={() => { if (onNavigateTab) onNavigateTab('research'); }}
                    className="w-full flex items-center justify-between p-2.5 bg-white border border-border-light rounded-md hover:border-[#10B981] hover:text-[#10B981] transition-colors group"
                  >
                    <div className="flex items-center gap-2.5">
                      <Search size={14} className="text-foreground-secondary group-hover:text-[#10B981]" />
                      <span className="text-[13px] font-medium text-[#0B1628] group-hover:text-[#10B981]">Find evidence</span>
                    </div>
                    <ArrowRight size={14} className="opacity-0 group-hover:opacity-100 transition-opacity" />
                  </button>

                  <button 
                    onClick={() => handleGenerateContextual("Review academic claims and tone")}
                    className="w-full flex items-center justify-between p-2.5 bg-white border border-border-light rounded-md hover:border-[#F97316] hover:text-[#F97316] transition-colors group"
                  >
                    <div className="flex items-center gap-2.5">
                      <ShieldCheck size={14} className="text-foreground-secondary group-hover:text-[#F97316]" />
                      <span className="text-[13px] font-medium text-[#0B1628] group-hover:text-[#F97316]">Review academic claim</span>
                    </div>
                    <ArrowRight size={14} className="opacity-0 group-hover:opacity-100 transition-opacity" />
                  </button>
                </>
      ...
      <div className="flex flex-col h-full bg-white relative font-sans">
        <div className="flex-1 p-5 overflow-y-auto space-y-8">
          ...
          <section className="mt-2">
            <h2 className="text-[15px] font-bold text-[#0B1628]">Project Context</h2>
            <p className="text-[13px] text-foreground-secondary mt-1 leading-relaxed">
              {(projectContext?.description as string) || "No specific research context set for this project."}
            </p>
          </section>
          ...
          <section>
            <h4 className="text-[11px] font-semibold text-foreground-muted uppercase tracking-wider mb-4">Document Stats</h4>
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col">
                <span className="text-[20px] font-semibold text-[#0B1628] leading-none">{wordCount || 0}</span>
                <span className="text-[12px] text-foreground-secondary mt-1">Words</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[20px] font-semibold text-[#0B1628] leading-none">{sourceCount || 0}</span>
                <span className="text-[12px] text-foreground-secondary mt-1">Sources</span>
              </div>
            </div>
          </section>
          ...
          <section>
            <h4 className="text-[11px] font-semibold text-foreground-muted uppercase tracking-wider mb-3">Academic Actions</h4>
            <div className="space-y-1">
              <button 
                onClick={() => onAnalyze()}
                className="w-full flex items-center justify-between px-3 py-2 text-left hover:bg-black/5 rounded transition-colors group"
              >
                <div className="flex items-center gap-2">
                  <ShieldCheck size={15} className="text-foreground-secondary group-hover:text-[#0B1628]" />
                  <span className="text-[13px] font-medium text-[#0B1628]">Run Full Document Review</span>
                </div>
              </button>
              <button 
              ...
              >
                <div className="flex items-center gap-2">
                  <BookOpen size={15} className="text-foreground-secondary group-hover:text-[#0B1628]" />
                  <span className="text-[13px] font-medium text-[#0B1628]">Search Research Sources</span>
                </div>
              </button>
            </div>
          </section>
